 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class TheAtlantic_Sniffs_Classes_ClassDeclarationSniff extends PEAR_Sniffs_Classes_ClassDeclarationSniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($braceLine === ($classLine + 1)) {
            $error = 'Opening brace of a %s must be on the same line as its definition';
            $phpcsFile->addError($error, $curlyBrace, 'OpenBraceSameLine', $errorData);
            return;
        } else if ($braceLine > ($classLine + 1)) {
            $error = 'Opening brace of a %s must be on the same line as its declaration; found %s line(s)';
            $data  = array(
        ...
        if ($tokens[($curlyBrace + 1)]['content'] !== $phpcsFile->eolChar) {
            $error = 'Opening %s brace must be followed by a new line';
    ...
    public function processClose(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        $classCloseLines = $tokens[$prevContent]['line'] - $tokens[$closeBrace]['line'];

        if ($classCloseLines == 0 || $classCloseLines > 2) {
            $error = 'The closing brace for the %s must be within two newlines of the end of the body';
